...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, sans-serif; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center; }
        body.authenticated { background: none; display: block; }
        ...
        h1 { color: #1a1a2e; font-size: 24px; font-weight: 600; margin-bottom: 8px; }
        ...
        #error { color: #dc3545; font-size: 14px; margin-top: 20px; padding: 12px; background: #fff5f5; border-radius: 8px; display: none; }
...
<body>
    ...
    <script>
        ...
        function signIn() { const provider = new firebase.auth.GoogleAuthProvider(); firebase.auth().signInWithPopup(provider).catch(error => { showError('Sign-in failed: ' + error.message); }); }
        ...
        function showDashboard() {
            ...
            if (encodedData && !encodedData.includes('PLACEHOLDER')) {
                try {
                    const dashboardHtml = atob(encodedData);
                    ...
                    doc.querySelectorAll('script').forEach(script => { const newScript = document.createElement('script'); if (script.src) { newScript.src = script.src; } else { newScript.textContent = script.textContent; } document.body.appendChild(newScript); });
